...
- **`/spec` exception:** plan approval authorizes the chain's per-story and review-fix commits (`git add <files>` + `git commit` on the current branch) — nothing else. Unrelated uncommitted changes → stop and ask first.
