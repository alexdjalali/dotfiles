...
- Tests are **contra-variant** with code structure and **structure-insensitive** — a behavior-preserving refactor keeps them green. No one-class-per-method, no redundant assertions on the same path, no test-per-trivial-helper (no branches, I/O, or public API ⇒ the caller's test covers it), no coverage-padding.
...
- A project may override this posture via `.claude/rules/testing-project.md` (e.g. a strict numeric coverage gate) — the **only** way to loosen the test-double policy.

## TDD — red / green / refactor (default)

1. **RED** — one minimal test for the desired behavior (behavior, not implementation). Mock external deps only. Naming: Python `test_<function>_<scenario>_<expected>` · TS `it("should <behavior> when <condition>")`.
...
Applies to new functions, endpoints, business logic, **bug fixes (reproduce first)**, behavior changes. **Skip RED** only for docs / config / dep bumps / formatting, or a planned `Trivial:` change that names its covering test/command and adds ≤ 5 net production lines with no new branch/loop/try (non-trivial body), no new public symbol, and no new error path — audited against the actual diff, not taken on trust. **Bugfixes never skip** the reproducing test. Code written first? Don't revert — write the test now and verify it catches regressions.
