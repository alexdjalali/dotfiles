...
local pages = {
  spec = {
    ...
    sections = {
      { "Flow", {
        "/adr → /design-doc → /rfp → /spec (epic or story) → /github",
        "decide → design/diagram → stories → plan, implement, verify → ship",
        "/design-doc arch       architecture diagrams only",
      } },
      { "Plan and build", {
        "/spec <epic|story|task>  plan → [you approve] → implement → verify",
        "/spec-plan  /spec-implement  /spec-verify     the phases (feature + bugfix)",
        "1 story = 1 commit; one review + one full gate at the end",
      } },
      { "Bugs", {
        "/fix                   reproduce, root-cause, test first, small fix",
        "/rca                   diagnosis only, file:line cited",
      } },
      { "Review and ship", {
        "/review-diff [deep]    one-pass review of any diff (on Fable)",
        "quality gates          fast checks per commit; full gate once, before push",
        "/github                commit, branch, PR, merge (each git write confirmed)",
      } },
      { "Artifacts and upkeep", {
        "/audit [standard|repo] quick sweep, standard audit, or layout check",
        "/sync-docs  /learn     repo-wide doc sweep; keep a reusable skill",
        "docs/adr, docs/spec/{arch,design,epics,stories,rca,audits}",
